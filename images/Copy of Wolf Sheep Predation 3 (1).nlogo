...
breed [ wolves wolf ]
breed [snakes snake]
turtles-own [ energy ]       ; snakes, wolves, and sheep have energy
...
to setup
  ...
  create-wolves initial-number-wolves  ; create the snakes, then initialize their variables
  [
    set color black
  ...
  set-default-shape snakes "line"
  create-snakes initial-number-snakes  ; create the snakes, then initialize their variables
  [
    set color orange
    set size 2  ; easier to see
    set energy random (1 *  snake-gain-from-food)
    setxy random-xcor random-ycor
  ]
  display-labels
...
to go
  ...
  if not any? snakes and count sheep > max-sheep [ user-message "The sheep have inherited the earth" stop ]
  ask sheep [
    move
    if grass? [
  ...
  ask wolves [
   ...
   ask snakes [
    move
    set energy energy - 1  ; snakes lose energy as they move
    catch-sheep
    death
    reproduce-snakes
  ]
...
to reproduce-snakes  ; snake procedure
  if random-float 100 < snake-reproduce [  ; throw "dice" to see if you will reproduce
    set energy (energy / 2)               ; divide energy between parent and offspring
    hatch 1 [ rt random-float 360 fd 1 ]  ; hatch an offspring and move it forward 1 step
  ]
end

to catch-sheep ; snake procedure
 let prey one-of sheep-here
  if prey != nobody
    [ ask prey [ die ]
      set energy energy + snake-gain-from-food ]
...
to display-labels
  ...
  if show-energy? [
    ...
    ask snakes [ set label round energy ]
...
"snakes" 1.0 0 -955883 true "" "plot count snakes"
